...
pre_asset_file <- file.path(output_dir, "analysis_sample_stage2_step5_pre_asset_complete_cases.rds")
stopifnot(file.exists(pre_asset_file))

data <- readRDS(pre_asset_file)
...
stopifnot(all(asset_cols %in% names(data)))

# Sensitivity scheme: NA in asset components -> 0.
data <- data %>%
  mutate(across(all_of(asset_cols), ~ ifelse(is.na(.x), 0, .x))) %>%
  mutate(
    ...
    risk_asset_total = rowSums(across(all_of(c("stock_amt", "fund_amt", "lend_amt")))),
    financial_asset_total = rowSums(across(all_of(asset_cols)))
  )
...
for (i in seq_len(nrow(comb_grid))) {
  ...
  d0 <- data %>% mutate(denominator = rowSums(across(all_of(denom_components))))
  ...
  for (risk_name in names(risk_specs)) {
    ...
    for (sv in sample_variants) {
      ...
      scenario_id <- paste(g$denom_spec_id, risk_name, sv, "impute0", sep = "__")
...
write.csv(res, file.path(tables_dir, "table2_ols_grid_search_impute0.csv"), row.names = FALSE)
...
write.csv(best10, file.path(tables_dir, "table2_ols_grid_top10_impute0.csv"), row.names = FALSE)

summary_lines <- c(
  sprintf("Generated at: %s", format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z")),
  sprintf("Pre-asset sample rows (step5): %d", nrow(data)),
...
writeLines(summary_lines, file.path(logs_dir, "baseline_ols_grid_search_impute0.log"))

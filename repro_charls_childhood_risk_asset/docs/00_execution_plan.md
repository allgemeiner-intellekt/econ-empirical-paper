...
- [x] 阶段 4：基准回归复现（表2）。目标：`Risk_Dummy` 用 Logit，`Risk_Ratio` 用 OLS，使用稳健标准误；产物：`src/03_baseline_models.*`、`output/tables/table2_reproduced.*`、`docs/04_baseline_results.md`（与论文系数方向和量级对照）。（完成日期：2026-02-27）

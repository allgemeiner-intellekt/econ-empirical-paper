...
- [x] 阶段 4：基准回归复现（表2）。目标：`Risk_Dummy` 用 Logit，`Risk_Ratio` 用 OLS，使用稳健标准误；产物：`src/03_baseline_models.*`、`output/tables/table2_reproduced.*`、`docs/04_baseline_results.md`（与论文系数方向和量级对照）。（完成日期：2026-02-27，含 448 场景增强复核）
- [x] 阶段 5：内生性处理复现（表3）。目标：以 `hospital` 为工具变量做 2SLS，报告第一阶段 F 值与二阶段结果；产物：`src/04_iv_2sls.*`、`output/tables/table3_reproduced.*`、`docs/05_iv_results.md`。（完成日期：2026-02-27，受限完成：官方 IV 定义受限，已补充 IV 候选扫描）

...
## 4.3 新增复核：替代/多 IV 的 2SLS 对照表（可复跑）

为使阶段 5 的结论更可复核并尽量逼近论文表 3 的“方向与显著性”，新增一个可复跑的对照脚本，统一输出不同 IV 设定下 (1)-(4) 的二阶段系数与第一阶段 F。

- 脚本：`src/04b_iv_sensitivity_alternative_ivs.R`
- 输出：
  - `output/tables/table3_iv_sensitivity.csv`
  - `output/tables/table3_iv_sensitivity_summary.csv`

对照结果（按“对论文表 3 的总绝对偏差”排序）：

1) **多 IV：`vaccinated_childhood + hospitalized_3plus`**（over-identified，敏感性分析）
- 二阶段 `childhealth` 系数（(1)-(4)）：`[-0.0687, -0.0478, -0.0415, -0.0322]`
- 显著性：四列均为负，且四列均达到 `p<0.1`
- 第一阶段：最小 F 约 `76.48`
- 解释：在“方向 + 显著性”层面与论文最一致，但该设定不等价于论文的单一 IV（`hospital`），应仅作为敏感性分析而非主复现。

2) **单 IV：`vaccinated_childhood`**（更接近“就医可及性”的可执行代理）
- 二阶段 `childhealth` 系数（(1)-(4)）：`[-0.4623, -0.1619, -0.2145, -0.0447]`
- 显著性：前三列显著为负，第 (4) 列不显著（`p≈0.36`）
- 第一阶段：最小 F 约 `12.35`
- 解释：方向与论文一致，但量级偏大（尤其 (1)/(3) 列）。

结论：在缺少论文同义 `hospital` 字段时，**单一候选 IV 的复现结果对 IV 设定高度敏感**；若仅追求“方向与显著性”一致，多 IV 设定可实现，但其识别假设更强、可解释性需谨慎。

...
## Local Survey Data (Data-First Workflow)

When users provide their own data files, use these R loading patterns.
...
label_df <- tibble(variable = names(var_labels), label = unlist(var_labels))
...
```r
library(haven)
df <- read_sav("data/survey.sav")
# Same label handling as Stata
```

### Excel (.xlsx)
```r
library(readxl)
sheets <- excel_sheets("data/survey.xlsx")
df <- read_xlsx("data/survey.xlsx", sheet = 1)
# Multi-sheet: map_dfr(sheets, ~read_xlsx("data/survey.xlsx", sheet = .x))
```

### Large CSV (>100MB)
```r
library(data.table)
df <- fread("data/large_survey.csv", encoding = "UTF-8")
# For Chinese data: encoding = "GBK" if UTF-8 produces garbled text
```

### Common Chinese Survey Datasets

| Dataset | Full Name | Type | Typical Format |
|---------|-----------|------|---------------|
| CFPS | China Family Panel Studies | Panel | .dta |
| CGSS | Chinese General Social Survey | Repeated cross-section | .dta / .sav |
| CHARLS | China Health and Retirement Longitudinal Study | Panel | .dta |
| CHNS | China Health and Nutrition Survey | Panel | .dta |
| CHIP | Chinese Household Income Project | Cross-section | .dta |

### Common US/International Survey Datasets

| Dataset | Full Name | Type | Typical Format |
|---------|-----------|------|---------------|
| CPS | Current Population Survey | Repeated cross-section | .dta (via IPUMS) |
| PSID | Panel Study of Income Dynamics | Panel | .txt / .sav |
| HRS | Health and Retirement Study | Panel | .dta |
| NLSY | National Longitudinal Survey of Youth | Panel | .dta |
| ATUS | American Time Use Survey | Cross-section | .dta (via IPUMS) |
| EU-SILC | EU Statistics on Income and Living Conditions | Panel | .csv |
